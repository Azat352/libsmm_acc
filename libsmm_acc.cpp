...
//int libsmm_acc_process_cutlass(const int *param_stack, int stack_size, ACC_DRV(stream) stream, int m, int n, int k, int max_kernel_dim, const double *a_data, const double *b_data, double *c_data) {
//
//   int istat = 0; 
//    for (int stack_entry = 0; stack_entry < stack_size; stack_entry++) {
//       cutlass_run(
//                &a_data[param_stack[7 * stack_entry + 3] - 1], 
//                &b_data[param_stack[7 * stack_entry + 4] - 1], 
//                &c_data[param_stack[7 * stack_entry + 5] - 1], 
//                &c_data[param_stack[7 * stack_entry + 5] - 1], 
//                m, n, k,
//                1.f, 1.f);
//    }
...
int libsmm_acc_process_wmma(const int *param_stack, int stack_size, ACC_DRV(stream) stream, int m, int n, int k, int max_kernel_dim, const double *a_data, const double *b_data, double *c_data) {
...
int libsmm_acc_process (const int* param_stack_host, const int *param_stack_dev, int stack_size, int nparams, libsmm_acc_data_t datatype, const void *a_data, const void *b_data, void *c_data, int m, int n, int k, int max_kernel_dim, int def_mnk, void *stack_stream, void *c_stream) {
    ...
    if ((m % 8 == 0) & (n % 8 == 0) & (k % 4 == 0))
         return  (libsmm_acc_process_cutlass ((const int *) param_stack_host, stack_size, *((ACC_DRV(stream) *) c_stream), m, n, k, max_kernel_dim, (const double *) a_data, (const double *) b_data, (double *) c_data));
